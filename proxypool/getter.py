...
class ProxyMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):  #new方法，动态添加属性，将方法名汇总起来，通过属性名称获取方法列表。
        count = 0
        attrs['__CrawlFunc__'] = []  #CrawlFunc检测当前所有方法名，如果
        for k, v in attrs.items():
            if 'crawl_' in k:     #如果方法名以crawl_开头，
                attrs['__CrawlFunc__'].append(k)  #然后将方法名赋值给CrawlFunc
                count += 1
...
class FreeProxyGetter(object, metaclass=ProxyMetaclass):
    def get_raw_proxies(self, callback):
        ...
        for proxy in eval("self.{}()".format(callback)):  #proxy是下边类的名称，用来执行下边方法。
